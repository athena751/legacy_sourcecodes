...
<!-- "@(#) $Id: controllerlistrefresh.jsp,v 1.2301 2005/12/16 05:22:53 xingh Exp $" -->
<html>
...
<% String diskArrayID=request.getParameter("diskArrayID");
   ...
   if(refreshBean.refresh(diskArrayID)!=0){
    ...
    if(refreshBean.setSpecialErrMsg()) {%>
     ...
     <%} else {%>
<h1 class="Error"><nsgui:message key="fcsan_componentdisp/controller/msg_refresh_failed"/></h1>
...
parent.location="<%=response.encodeURL("controllerlist.jsp")%>?diskArrayID=<%=diskArrayID%>&diskname=<%=diskname%>"
</script>
<%}%>