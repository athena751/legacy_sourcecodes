...
<!-- "@(#) $Id: expandrank.jsp,v 1.2301 2004/07/14 08:42:44 nsadmin Exp $" -->
...
<jsp:useBean id="expandrankBean" class="com.nec.sydney.beans.fcsan.componentconf.RankExpandingBean" scope="page"/>
<% AbstractJSPBean _abstractJSPBean = expandrankBean; %>
<%@include file="../../../menu/common/includeheader.jsp" %>
<html>
<head>
<%int result = expandrankBean.getResult();
  if( result == 0){%>
...
<form name="expandRankFrom" method="post">    
      <%if(result==0){%>
    ...
    <%}else{%>
        <%if(expandrankBean.setSpecialErrMsg()) {%>
            <h1 class="popup"><nsgui:message key="common/alert/failed"/></h1>
            <h2 class="popup"><%=expandrankBean.getErrMsg()%></h2>
        <%}else{%>
            <h1 class="popupError"><nsgui:message key="common/alert/failed"/></h1>
            <h2 class="popupError"><%=expandrankBean.getErrMsg()%></h2>
        <%}%>
    <%}%>
<center> 
