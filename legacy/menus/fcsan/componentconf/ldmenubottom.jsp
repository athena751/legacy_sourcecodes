...
<!-- "@(#) $Id: ldmenubottom.jsp,v 1.2 2005/09/21 04:49:10 wangli Exp $" -->
...
function bindunbind(button)
{
    if(button.disabled)
        return;
    if(parent.frames[1].document.forms[0].diskarrayid.value!="")
        parent.location="<%=response.encodeURL("ldbindunbind.jsp")%>?diskarrayid="+parent.frames[1].document.forms[0].diskarrayid.value+"&diskarrayname="+parent.frames[1].document.forms[0].diskarrayname.value+"&arraytype="+parent.frames[1].document.forms[0].arraytype.value;

}

function checkradio()
{
    document.forms[0].ldbindunbind.disabled = true;
    ...
    if(parent.frames[1].document.forms[0]
        ...
        if (parent.frames[1].document.forms[0].observation.value != "<%=FCSANConstants.FCSAN_STATE_NO_MONITORING%>") { 
            document.forms[0].ldbindunbind.disabled=false;
...
<form>
    <input type="button" name="ldbindunbind" value="<nsgui:message key="fcsan_componentconf/confmenu/button_lbu"/>" onclick="bindunbind(this)">
</form>
</body>
</html>
