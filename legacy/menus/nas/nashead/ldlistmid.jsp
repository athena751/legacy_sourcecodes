...
function loadbottomframe(){
    parent.bottomframe.location = "<%=response.encodeURL("ldlistbottom.jsp")%>";
}
function reloadall(){
    var info = browserInfo();
    if( info == "WNS4" || info == "LNS4" ){
        window.parent.location ="<%=response.encodeURL("ldlist.jsp")%>";
    }
}

</script>
</head>
<body onload="displayAlert();loadbottomframe();" onResize="reloadall();">
<%String forH3="";
...
<%}else{
    ...
    <%}else{
        ...
        %>
        <tr>
            <td align="center"><input type="radio" name="lunRadio" <%=checked%> id="storageID<%=j%>" value="<%=lunForShow%>,<%=lun.getDevicePath()%>,<%=lun.getLvm()%>,<%=lun.getPairStatus()%>" onClick="selectlun(this);"></td>
