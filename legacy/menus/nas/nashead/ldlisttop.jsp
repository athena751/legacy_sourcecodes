...
<!-- "@(#) $Id: ldlisttop.jsp,v 1.7 2005/10/24 06:56:56 liuyq Exp" -->
...
function goToAdd (){
    ...
    var confirMessage = "<nsgui:message key="common/confirm"/>" + "\r\n"
		     + "<nsgui:message key="common/confirm/act"/>"
		     + "<nsgui:message key="nas_nashead/common/batchset_button"/>" + "\r\n"
		     + "<nsgui:message key="nas_nashead/alert/link_confirm"/>";
    if(confirm(confirMessage)){
        setSubmitted();
        var date = new Date();
        var starttime = date.getTime();
        popHeartBeat=window.open("<%=response.encodeURL("../../common/heartbeat.jsp")%>?frameNo=0&starttime="+starttime,"heartbeat","resizable=no,statusbar=no,locationbar=no,menubar=no,toolbar=no,scrollbar=no,width=400,height=150");
        parent.location="ldAutoLink.jsp?EXTEND_INACTIVE_INTERVAL=5000";
        return true;
    }else{
        return false;
    }
...
function goToChange(){
    ...
    if(document.storageInfo.wwnn.value == ""){
        alert("<nsgui:message key="common/alert/failed"/>" + "\r\n"
                   +"<nsgui:message key="nas_nashead/alert/nostorageselect"/>");
        return false;
    }
    setOperation(document.storageInfo,'');
    document.storageInfo.action="ldModifyStorageName.jsp";
    document.storageInfo.target="ACTION";
...
function getLunList(){
    ...
    document.storageInfo.target="midframe";
    document.storageInfo.action="ldlistmid.jsp";
...
function reloadPage(){
    ...
    setSubmitted();
    parent.location="ldlist.jsp";         
}    
</script>
<jsp:include page="../../common/wait.jsp" />
</head>

<body onload="displayAlert();getLunList();" onUnload="closeHeartBeat();">
...
<%}else{%>
	<input type="button" name="newAdd1" value="<nsgui:message key="nas_nashead/common/init_button"/>" onClick="parent.location='luncreate.jsp?src=init'">
	<input type="button" name="newAdd2" value="<nsgui:message key="nas_nashead/common/batchset_button"/>" onClick="parent.location='luncreate.jsp?src=link'">
    ...
    <%}else{
        ...
        <input type="button" name="storageChange"  value="<nsgui:message key="nas_nashead/common/changenickname_button"/>" onclick="goToChange()">
    <%}
}
%>
</form>
