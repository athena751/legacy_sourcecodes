...
<script language="JavaScript">
    ...
    function setBasicInfo() {
        if(isSubmitted()){
            document.formHTTPInfo.action="<%=response.encodeURL("httpbasiccfg.jsp")%>";
            document.formHTTPInfo.submit();
            setSubmitted();
        }
    }

    function setServerInfo(btn) {
        if(isSubmitted()){
            if (btn.name=="btnSetMainSvrInfo") {
                document.forms[0].operation.value="MainServerEdit" ;
            } else if (btn.name=="btnAddVtrlHostInfo") {
                document.forms[0].operation.value="VirtualHostAdd" ;
            } else if (btn.name=="btnEditVtrlHostInfo") {
                document.forms[0].operation.value="VirtualHostEdit" ;
            }
            document.formHTTPInfo.action="<%=response.encodeURL("httpservercfg.jsp")%>";
            document.formHTTPInfo.submit();
            setSubmitted();
        }
    }
    ...
    function setAllInfo(){
        if(isSubmitted()){
            document.formHTTPInfo.action="<%=response.encodeURL("../../../menu/common/forward.jsp")%>";
            document.forms[0].operation.value="doConfig";
            document.forms[0].beanClass.value="<%=HTTPInfo.getClass().getName()%>";
            document.formHTTPInfo.submit();
            setSubmitted();
        }
    }
    function onReloadInfo(){
        if(isSubmitted()){
            ...
            window.location="<%=response.encodeURL("httpinfo.jsp")%>";
            setSubmitted();
        }
    }
</script>
...
%>
<form name="formHTTPInfo" method="post" onSubmit="checkOperation()">
<input type="hidden" name="operation" value="">
<input type="hidden" name="beanClass" value="">
...
<input type="button" name="set" value="<nsgui:message key="nas_http/button/set"/>" onclick="setAllInfo()">


<h2><nsgui:message key="nas_http/basic/h2"/></h2>
<input type="button" name="btnSetBasicInfo" value="<nsgui:message key="nas_http/button/modify"/>" onClick="setBasicInfo()">
<br><br>
<table border="1">
<tr>
...
<input type="button" name="btnSetMainSvrInfo" value="<nsgui:message key="nas_http/button/modify"/>"
       onclick=" return setServerInfo(this)">
<br><br>
<table border="1">
<tr>
...
<% } %>
<input type="button" name="btnAddVtrlHostInfo" value="<nsgui:message key="nas_http/button/add"/>"
       onclick=" return setServerInfo(this)">
<% if (theHostInfo.size()!=0) { %>
<br><br>
<table border="1">
<tr>
    ...
    <tr>
        <td><input type="radio" name="nickName"
                   value="<%= ((HTTPServerInfo)theHostInfo.get(i)).getNickName() %>"
                   <%if(i==0) out.print("checked") ;%> ></td>
...
<br>
<input type="button" name="btnEditVtrlHostInfo" value="<nsgui:message key="nas_http/button/edit"/>"
       onclick="return setServerInfo(this)">
<% } %>
