...
<!-- "@(#) $Id: exportRootTop.jsp,v 1.9 2009/01/12 10:24:02 xingyh Exp $" -->
...
<script language="JavaScript">
    var loaded = 0;
    var strCurBusyExport = "<%=ExportRootBean.getCreatingVolInExport(request)%>";
    function topFrameInit(){
        ...
        <%if(exportNumber==0){%>
            if(parent.frames[1].loaded){
                ...
                parent.frames[1].document.forms[0].btnDelete.disabled=1;
            }
        <%}else{%>
            if(parent.frames[1].loaded){
                parent.frames[1].document.forms[0].btnDetail.disabled=0;
                parent.frames[1].document.forms[0].btnModify.disabled=0;
                parent.frames[1].document.forms[0].btnDelete.disabled=0;
            }
        <%}%>
    }
    ...
    function onCreate(){
        if(document.forms[0].hasExportGroup){
            parent.location = "<%=response.encodeURL("exportrootcreate.jsp?hasExportGroup=1")%>";
        }else{
            parent.location = "<%=response.encodeURL("exportrootcreate.jsp?hasExportGroup=0")%>";
        }
        return false;
    }
    ...
    function reloadPage() {
        ...
        setSubmitted();     
        window.location="<%=response.encodeURL("exportRootTop.jsp")%>";
    }    
</script>
</HEAD>
<BODY onload="topFrameInit();displayAlert();setHelpAnchor('export_setup')">
...
<table border="0">
    <tr>
        <td>
        <nsgui:message key="nas_exportroot/exportroot/create_notes"/>
        </td>
    </tr>
</table> 
    <p>
    <input type="button" name="addButton" value="<nsgui:message key="common/button/create2"/>" onclick="return onCreate()" />
    </p>
