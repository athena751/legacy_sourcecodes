...
<!-- "@(#) $Id: vcnavigatorsubmit.jsp,v 1.7 2007/09/13 01:17:48 pizb Exp $" -->
...
function submitCheck() {
    ...
    if(document.forms[0].displayedDirectory.value != ""){
        if (checkName(document.forms[0].displayedDirectory.value)) {
            alert("<bean:message key="common.alert.failed" bundle="common"/>" + "\r\n" 
                + '<bean:message key="msg.navigator.alert.invalid_name" arg0="\'+disDir+\'" />');
            return false;
        }
    }
    ...
    if (fullPath.length > 2047){
        alert("<bean:message key="common.alert.failed" bundle="common"/>" + "\r\n" 
             + "<bean:message key="msg.add.maxMPLength"/>");
        return false;                
    }
    
    if (chkEveryLevel(disDir)){
        alert("<bean:message key="common.alert.failed" bundle="common"/>" + "\r\n" 
            + "<bean:message key="msg.add.maxDirLength"/>");
