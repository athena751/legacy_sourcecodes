...
#
# "@(#) $Id: cifs_saveSmbConf.pl,v 1.7 2007/06/28 01:43:22 fengmh Exp $"
...
if($needChangeCode eq "y") {
    system("cat ${tempFile} | $nsguiIconv -f UTF-8 -t UTF8-NEC-JP | $cmd_syncwrite_o ${smb_conf_File}");
} else {
    system("${cmd_syncwrite_m} ${tempFile} ${smb_conf_File}");
}

if($? != 0){
    system("rm -f ${tempFile}");
    $cvs->rollback($smb_conf_File);
    $comm->writeErrMsg($const->ERRCODE_FAILED_TO_WRITE_SMB_CONF_FILE,__FILE__,__LINE__+1);
    exit 1;
}
system("rm -f ${tempFile}");
my $cmd = "/home/nsadmin/bin/ns_nascifsstart.sh";
if(system("$cmd") != 0){
    #system("rm -f ${tempFile}");
    #$cvs->rollback($smb_conf_File);
    #$comm->writeErrMsg($const->ERRCODE_FAILED_TO_EXEC_NASCIFSSTART,__FILE__,__LINE__+1);
    #exit 1;
}
